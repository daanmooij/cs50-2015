...
int main(int argc, char* argv[])
{
    ...
    while (true)
    {
        ...
        if (connected())
        {
            ...
            char rqline[needle - haystack + 1];
            strncpy(rqline, haystack, needle - haystack);
            rqline[needle - haystack] = '\0';
            
            
            
            // extract tokens from request-line
            char* tokens[3];
            tokens[0] = strtok(rqline, " ");
            ...
            if(strcmp("GET", tokens[0]) != 0 || tokens[0] == NULL)
            {
                error(405);
                continue;
            }
            ...
            if(strcmp("HTTP/1.1",tokens[2]) != 0 || tokens[2] == NULL)
            {
                error(505);
                continue;
            }
            ...
            char* qmark = strchr(tokens[1], '?');
            char* delim = strchr(tokens[1], '\0');
                        
            char query[(delim - qmark < 1) ? 1 : delim - qmark];
            ...
            {
                strncpy(query, qmark + 1, delim - qmark - 1);
                query[delim - qmark - 1] = '\0';
            }
            
            // get absolute path
            char abspath[(delim - qmark < 1) ? delim - tokens[1] + 1 : qmark - tokens[1] + 1];
            
            if(delim - qmark < 1)
            {
                strncpy(abspath, tokens[1], delim - tokens[1]);
                abspath[delim - tokens[1]] = '\0';
            }
            else
            {
                strncpy(abspath, tokens[1], qmark - tokens[1]);
                abspath[qmark - tokens[1]] = '\0';
            }
            ...
            delim = strchr(abspath, '\0');
            char* dot = strrchr(abspath, '.');
            char extension[delim - dot];
            strncpy(extension, dot + 1, delim - dot - 1);
            extension[delim - dot - 1] = '\0';
